...
class MinimumNumber {
    public static void main(String[] args) {
        ...
        int res = minNumberOfOperations(nums, x, 0, nums.length-1, 0, map);
        ...
        int res2 = minNumberOfOperations(nums2, x2, 0, nums2.length-1, 0, map2);
        ...
        int res3 = minNumberOfOperations(nums3, x3, 0, nums3.length-1, 0, map3);
    ...
    static int minNumberOfOperations(int[] nums, int x,int left, int right, int count, HashMap<String, Integer> map) {
        if(x==0) {
            map.put(String.valueOf(left*right*count), count);
            return count;
        }
        if(left>=right || x < 0) {
        ...
        int leftRes = minNumberOfOperations(nums, x-nums[left], left+1, right, count+1, map);
        int rightRes = minNumberOfOperations(nums, x-nums[right], left, right-1, count+1, map);
        map.put(key, Math.min(leftRes, rightRes));
